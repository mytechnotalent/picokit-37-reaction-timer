/**
 * FILE: test_picokit_37_reaction_timer_and_security.c
 *
 * DESCRIPTION:
 * Native unit tests for the RP2350 Picokit reaction timer node: provisioning
 * constants, the packet artifact, CRC, the RYLR998 AT interface, the 1602
 * LCD, the reaction button, and the random-light reaction state machine.
 *
 * BRIEF:
 * Native unit test runner for picokit-37-reaction-timer.
 *
 * AUTHOR: Kevin Thomas
 * DATE: September 2026
 */
...
static void reset_all(void) {
    ...
    mock_rand_reset();
    button_reset();
    gpio_put(PICOKIT_37_REACTION_TIMER_BUTTON_PIN, true);
}
...
/**
 * @brief Drive the reaction button into the pressed state.
 *
 * @param void No parameters.
 * @return void
 */
static void press_button(void) {
    gpio_put(PICOKIT_37_REACTION_TIMER_BUTTON_PIN, false);
}

/**
 * @brief Drive the reaction button into the released state.
 *
 * @param void No parameters.
 * @return void
 */
...
static void assert_pin_constants(void) {
    TEST_ASSERT_EQUAL_UINT(25u, PICOKIT_37_REACTION_TIMER_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(8u, PICOKIT_37_REACTION_TIMER_UART_TX);
    TEST_ASSERT_EQUAL_UINT(9u, PICOKIT_37_REACTION_TIMER_UART_RX);
    TEST_ASSERT_EQUAL_UINT(16u, PICOKIT_37_REACTION_TIMER_RED_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(17u, PICOKIT_37_REACTION_TIMER_YELLOW_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(18u, PICOKIT_37_REACTION_TIMER_GREEN_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(15u, PICOKIT_37_REACTION_TIMER_BUTTON_PIN);
}

/**
 * @brief Assert the I2C LCD and frame provisioning constants.
 *
 * @param void No parameters.
 * @return void
 */
static void assert_frame_constants(void) {
    TEST_ASSERT_EQUAL_UINT(2u, PICOKIT_37_REACTION_TIMER_I2C_SDA);
    TEST_ASSERT_EQUAL_UINT(3u, PICOKIT_37_REACTION_TIMER_I2C_SCL);
    TEST_ASSERT_EQUAL_UINT(115200u, PICOKIT_37_REACTION_TIMER_UART_BAUD);
    TEST_ASSERT_EQUAL_UINT(48u, PICOKIT_37_REACTION_TIMER_FRAME_SIZE);
    TEST_ASSERT_EQUAL_UINT(5000u, PICOKIT_37_REACTION_TIMER_TX_INTERVAL_MS);
...
static void assert_artifact_ids(void) {
    ...
    TEST_ASSERT_EQUAL_UINT(37u, PACKET_NODE_ID);
...
static void assert_artifact_example(void) {
    ...
    TEST_ASSERT_EQUAL_UINT8(0x37u, PACKET_EXAMPLE_FRAME[6]);
}

/**
 * @brief Assert the tail bytes of the packet artifact example frame.
 *
 * @param void No parameters.
 * @return void
 */
static void assert_artifact_example_tail(void) {
    TEST_ASSERT_EQUAL_UINT8(0x32u, PACKET_EXAMPLE_FRAME[18]);
    TEST_ASSERT_EQUAL_UINT8(0x34u, PACKET_EXAMPLE_FRAME[19]);
    TEST_ASSERT_EQUAL_UINT8(0x30u, PACKET_EXAMPLE_FRAME[20]);
    TEST_ASSERT_EQUAL_UINT8(0x7Du, PACKET_EXAMPLE_FRAME[21]);
    TEST_ASSERT_EQUAL_UINT8(0x00u, PACKET_EXAMPLE_FRAME[22]);
}
...
void test_monitor_build_frame_json(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("{\"n\":37,\"s\":12,\"m\":240}", frame);
}
...
void test_monitor_init(void) {
    ...
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_37_REACTION_TIMER_LED_PIN]);
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[PICOKIT_37_REACTION_TIMER_BUTTON_PIN]);
    TEST_ASSERT_TRUE(mock_i2c_log_count() > 0u);
}

void test_monitor_arm(void) {
    init_monitor();
    TEST_ASSERT_TRUE(g_armed);
    TEST_ASSERT_EQUAL_INT(0, mock_gpio_get(PICOKIT_37_REACTION_TIMER_LED_PIN));
}

void test_monitor_light(void) {
    uint64_t base;
    init_monitor();
    base = mock_timer_now_us();
    step_at(base + 600000u);
    TEST_ASSERT_FALSE(g_armed);
    TEST_ASSERT_EQUAL_INT(1, mock_gpio_get(PICOKIT_37_REACTION_TIMER_LED_PIN));
}

void test_monitor_reaction(void) {
    uint64_t base;
    init_monitor();
    base = mock_timer_now_us();
    step_at(base + 600000u);
    press_button();
    step_at(base + 800000u);
    TEST_ASSERT_EQUAL_UINT(200u, (unsigned)g_millis);
    TEST_ASSERT_TRUE(g_armed);
}

void test_monitor_miss(void) {
    uint64_t base;
    init_monitor();
    base = mock_timer_now_us();
    step_at(base + 600000u);
    step_at(base + 3700000u);
    TEST_ASSERT_EQUAL_UINT(MONITOR_PRESS_TIMEOUT_MS, (unsigned)g_millis);
    TEST_ASSERT_TRUE(g_armed);
}
...
/**
 * @brief Run the debounced reaction button tests.
 *
 * @param void No parameters.
 * @return void
 */
...
/**
 * @brief Run the reaction timer state-machine tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_monitor_tests(void) {
    ...
    RUN_TEST(test_monitor_arm);
    RUN_TEST(test_monitor_light);
    RUN_TEST(test_monitor_reaction);
    RUN_TEST(test_monitor_miss);
    RUN_TEST(test_monitor_transmit_frame);
    RUN_TEST(test_monitor_transmit_no_key);
}

/**
 * @brief Run the monitor readiness, bus, and inbound line tests.
 *
 * @param void No parameters.
 * @return void
 */
