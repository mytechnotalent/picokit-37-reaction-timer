...
static void assert_lamps(int red, int yellow, int green) {
    TEST_ASSERT_EQUAL_INT(red, mock_gpio_get(PICOKIT_37_REACTION_TIMER_RED_LED_PIN));
    TEST_ASSERT_EQUAL_INT(yellow,
                          mock_gpio_get(PICOKIT_37_REACTION_TIMER_YELLOW_LED_PIN));
    TEST_ASSERT_EQUAL_INT(green,
                          mock_gpio_get(PICOKIT_37_REACTION_TIMER_GREEN_LED_PIN));
}
...
static void test_status_led_init(void) {
    ...
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_37_REACTION_TIMER_RED_LED_PIN]);
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_37_REACTION_TIMER_YELLOW_LED_PIN]);
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_37_REACTION_TIMER_GREEN_LED_PIN]);
}
 ...
 */
static const uint8_t s_env_pt[] = "{\"n\":37,\"s\":0,\"m\":240}";
 ...
 */
static const char s_env_known_hex[] = "000102030405060708090a0b0c0d0e0f1011121314151617e5e0615daae1ba82113704f4fb7e8a85697d1a91cdabd176bdfd690febd1b300f505da5a1d44";
...
static void test_display_init(void) {
    reset_pc();
    TEST_ASSERT_TRUE(display_init(i2c1, PICOKIT_37_REACTION_TIMER_LCD_ADDR));
...
static void test_display_init_fails(void) {
    ...
    TEST_ASSERT_FALSE(display_init(i2c1, PICOKIT_37_REACTION_TIMER_LCD_ADDR));
}
...
static void test_display_show_lines(void) {
    reset_pc();
    display_init(i2c1, PICOKIT_37_REACTION_TIMER_LCD_ADDR);
    mock_i2c_reset();
    display_show_lines(i2c1, PICOKIT_37_REACTION_TIMER_LCD_ADDR, "HELLO", "WORLD");
    TEST_ASSERT_TRUE(mock_i2c_log_count() > 0u);
}
